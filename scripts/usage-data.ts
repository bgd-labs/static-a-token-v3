import { Hex, PublicClient, createPublicClient, getContract, http } from "viem";
import {
  arbitrum,
  mainnet,
  optimism,
...
async function checkUsageData(chain: Chain, factory: Hex, rpc?: string) {
  ...
  const tokens = await factoryContract.read.getStaticATokens();
  ...
  for (const token of tokens) {
    ...
    console.log(chain.name, symbol, totalSupply, token);
  }
}
